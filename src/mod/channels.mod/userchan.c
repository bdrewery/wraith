...
void write_bans(Stream& stream, int idx)
{
  ...
  for (b = global_bans; b; b = b->next) {
    ...
    if (mask) {
      stream.printf("- %s:%s%lu%s:+%lu:%lu:%s:%s\n", mask,
...
void write_exempts(Stream& stream, int idx)
{
  ...
  for (e = global_exempts; e; e = e->next) {
    ...
    if (mask) {
      stream.printf("%s %s:%s%lu%s:+%lu:%lu:%s:%s\n", "%", mask,
...
%cfastop %cautoop %cbotbitch %crelay %cnomassjoin %cbackup %cknock %c%s}\n",
	chan->dname,
	w,
        chan->added_by,
        chan->added_ts,
...
bool expired_mask(struct chanset_t *chan, char *who)
{
  ...
  if (m->user)
    ...
    simple_snprintf(buf, sizeof(buf), "%s!%s", m->nick, m->client->GetUHost());
