...
static void
detect_offense(Member* m, struct chanset_t *chan, char *msg)
{
  struct flag_record fr = { FR_GLOBAL | FR_CHAN, 0, 0, 0 };
  struct userrec *u = m->user ? m->user : get_user_by_host(m->client->GetUHost());
  int i = 0;
...
void detected_drone_flood(struct chanset_t* chan, Member* m) {
...
void makecookie(char *out, size_t len, const char *chname, const Member* opper, const Member* m1, const Member* m2, const Member* m3) {
  ...
  egg_snprintf(ts, sizeof(ts), "%010li", (long) (now + timesync));		/* &ts[4] is now last 6 digits of time */
...
getin_request(char *botnick, char *code, char *par)
{
  ...
  if (what[0] == 'o') {
    ...
    if (!mem->user && !mem->tried_getuser) {
      simple_snprintf(uhost, sizeof(uhost), "%s!%s", nick, mem->client->GetUHost());
